...
function clearUserSession() {
  ...
  if (typeof clearMealsSession === 'function') clearMealsSession();
}
...
if (firebaseEnabled) {
  onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
      ...
      if (typeof initMeals === 'function') initMeals();
      switchScreen(true);
...
function initMeals() {
  ...
  const mealsData = SafeStorage.getItem(`aura-logged-meals_${currentUser.uid}`);
  if (mealsData) {
    try {
      loggedMeals = JSON.parse(mealsData);
    } catch (e) {
...
function saveMealsState() {
  if (currentUser) {
    SafeStorage.setItem(`aura-logged-meals_${currentUser.uid}`, JSON.stringify(loggedMeals));
  }
}

// Update summary dashboard counters and progress bars
function updateSummaryUI(calories, protein, carbs, fat) {
  // Update texts
...
// Log a new meal
function logMeal(name, type, calories, protein, carbs = 0, fat = 0) {
  const newMeal = {
...
function bindMealsEvents() {
  ...
  quickLogButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      // Determine type based on recommended meal data
      let type = 'צהריים';
